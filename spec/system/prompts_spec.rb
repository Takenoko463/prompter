...
RSpec.describe 'Prompts', type: :system do
  ...
  describe 'prompt投稿ページを訪ねる' do
    context '投稿ページ表示に成功する' do
      it '投稿ページに初めて訪れる' do
        ...
        expect(page).to have_selector 'h1', text: 'プロンプト投稿ページ'
        expect(page).to have_content(@category_root.name)
      end
      it '投稿ページ2回目' do
        # sessionに現在のipを記録している 現在はブラウザを閉じると消去される。　後日、sessionを3日保存する設定を導入予定。
  ...
  describe 'prompt投稿を行う' do
    before '事前にトップページを訪ねている' do
      @ip = FactoryBot.create(:ip)
      @prompt = FactoryBot.build(:prompt, ip: @ip, category: @category_grand_child)
      visit root_path
    end
    context 'prompt投稿に成功する' do
      it 'root_categoryページから遷移してprompt投稿に成功する' do
        ...
        expect(page).to have_content(@prompt.nick_name)
      end
    ...
    context '投稿できない' do
      it '不備のある内容を投稿する' do
        ...
        sleep 1
        # トップページに遷移していることを確認する
        expect(page).to have_current_path(new_prompt_path)
      end
    end
  end
end
